...
class GetCoinsViewModel @Inject constructor(
    ...
    private var _listState = MutableStateFlow<List<Coin>>(emptyList())
    val listState: StateFlow<List<Coin>> = _listState

//    private var _listState = MutableSharedFlow<List<Coin>>()
//    val listState: SharedFlow<List<Coin>> = _listState
        ...
        viewModelScope.launch {
            getCoins().collect {
                when(it){
                    ...
                    is Resource.Success -> _listState.value = it.data ?: emptyList()
                }
            }
        }
